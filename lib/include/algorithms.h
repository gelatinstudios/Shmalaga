...
static inline int collision_detect(SDL_Rect rect1, SDL_Rect rect2) {
        int x = (rect1.x < rect2.x + rect2.w) && (rect1.x + rect1.w > rect2.x);
        int y = (rect1.y < rect2.y + rect2.h) && (rect1.y + rect1.h > rect2.y);
        return x && y;
}

static inline int is_on_screen(SDL_Rect rect) {
        int x = rect.x + rect.w > 0 && rect.x < 1280;
        int y = rect.y + rect.h > 0 && rect.y < 720;
        return x && y;
}

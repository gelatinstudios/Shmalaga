...
void automata(GameData *data, Sounds *sounds) {
        ...
        if(data->gamestate == IN_GAME) {
                ...
                move_enemies(data, sounds);

                for(size_t i = 0; i < data->gb_count; ++i)
                        data->gb[i].x += BULLET_VEL;

                for(size_t i = 0; i < data->bb_count; ++i) {
                        data->bb[i].rect.x -= rint(data->bb[i].dx);
                        data->bb[i].rect.y -= rint(data->bb[i].dy);
                }

                while(data->bb_count && !is_on_screen(data->bb[0].rect))
                        destroy_bb(data, 0);
        }
}
...
static inline void move_enemies(GameData *data, Sounds *sounds) {
        for(size_t i = 0; i < data->enemy_count; ++i) {
                ...
                if(is_on_screen(data->enemies[i].rect) && !rng(800)) {
                        create_bb(data, &data->enemies[i], sounds);
                }
        }

        if(data->gold_enemy.status == ALIVE) data->gold_enemy.rect.x -= 1.5*ENEMY_VEL;

        if(data->level == 7) {
                if(data->boss.rect.x > 2*1280/3)
                        data->boss.rect.x -= ENEMY_VEL;
        }
}

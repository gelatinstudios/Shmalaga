...
const SDL_Rect mascot_rect = {1280/2 - 300/2, 20, 300, 300};
const SDL_Rect studios_rect = {1280/2 - 1000/2, 350, 1000, 125};
const SDL_Rect presents_rect = {1280/2 - 600/2, 500, 600, 125};

static inline void render_stars(Star *stars, SDL_Renderer *rend) {
...
int intro(Star *stars, SDL_Renderer *rend, TTF_Font *font) {
        ...
        while(flag < 3) {
                ...
                if(!flag) ++i;
                else if (flag == 1) --timer;
                else if(flag == 2) --i;

                if(!flag && i > INTRO_LENGTH_1 * 255 / i) {
                         flag = 1;
                         --i;
                } else if(flag == 1 && !timer) flag = 2;
                else if(flag == 2 && !i) flag = 3;
...
SKIP:   SDL_DestroyTexture(mascot_text);
        ...
        while(timer) {
                ...
                const SDL_Rect title_rect = {1280/2 - width/2, 720/2 - height/2, width, height};

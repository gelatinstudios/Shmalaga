...
static inline void render_key(SDL_Renderer *, TTF_Font *, const SDL_Rect *, const char *, const char *, Uint8, Uint8);
static inline void render_volume(SDL_Renderer *, TTF_Font *, const SDL_Rect *, const char *, Uint8, int);
                ...
                case SOUND:
                        ...
                        if(state[SDL_SCANCODE_LEFT]) {
                                switch(data->selected) {
                                        case 0: Mix_Volume(-1, Mix_Volume(-1, -1) - VOL_SCALE);
                                                Mix_VolumeMusic(Mix_VolumeMusic(-1) - VOL_SCALE);
                                                break;
                                        case 1: Mix_VolumeMusic(Mix_VolumeMusic(-1) - VOL_SCALE); break;
                                        case 2: Mix_Volume(-1, Mix_Volume(-1, -1) - VOL_SCALE);
                                                break;
                                }
                        }
...
void render_menu(GameData *data, SDL_Renderer *rend, TTF_Font *font, SDL_Texture *menu[]) {
        ...
        switch(data->gamestate) {
                ...
                case SOUND: {
                        ...
                        render_volume(rend, font, &master, "MASTER VOLUME", data->selected == 0, MAX(Mix_Volume(-1, -1), Mix_VolumeMusic(-1)));
                        render_volume(rend, font, &music, "MUSIC VOLUME", data->selected == 1, Mix_VolumeMusic(-1));
                        render_volume(rend, font, &sfx, "SFX VOLUME", data->selected == 2, Mix_Volume(-1, -1));

                        char mute_str[16] = "MUTE: ";
                        strcat(mute_str, data->muted ? "SOUND OFF" : "SOUND ON");
                        SDL_Surface *mute_surf;
                        if(data->selected == 3) mute_surf = TTF_RenderText_Shaded(font, mute_str, black, white);
...
static inline void render_key(SDL_Renderer *rend, TTF_Font *font, const SDL_Rect *rect, const char *action, const char *keyname, Uint8 selected, Uint8 mode) {
        char str[50] = {0};
        sprintf(str, "%s: %s", action, keyname);

        SDL_Surface *surf;
        if(mode == KEYSET_MODE && selected) surf = TTF_RenderText_Shaded(font, "PRESS A KEY", black, white);
        else if(selected) surf = TTF_RenderText_Shaded(font, str, black, white);
        else surf = TTF_RenderText_Blended(font, str, white);
        render_surf(rend, surf, rect);
}

static inline void render_volume(SDL_Renderer *rend, TTF_Font *font, const SDL_Rect *rect, const char *barname, Uint8 selected, int volume) {
        char bar[] = "-----------";
        ...
        SDL_Surface *surf;
        if(selected) surf = TTF_RenderText_Shaded(font, str, black, white);
        else surf = TTF_RenderText_Blended(font, str, white);
        render_surf(rend, surf, rect);
}
